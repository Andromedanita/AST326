...
while m < len(max_x):
    x=max_x[m]
    y=max_y[m]
    box_x=np.arange(-5,+5,1)
    box_y=np.arange(-5,+5,1)
    multi_x=[]
    multi_y=[]
    denominator=[]
    for i in box_x:
        for j in box_y:
            multi_x.append((x+i)*corrected_value[y+j][x+i])
            multi_y.append((y+j)*corrected_value[y+j][x+i])
            ...
            #print centroids_x
        centroids_x.append(np.sum(multi_x)/np.sum(denominator))
        centroids_y.append(np.sum(multi_y)/np.sum(denominator))
print centroids_x, centroids_y
plt.plot(centroids_x, centroids_y,'*r')
