...
Here, $\Delta t$ is the time between the two limbs of the sun which is twice the $\Delta t$ we used to plot the limb darkening, therefore, $\Delta t$ is 126.138 s, which is equal to 2.1023 minutes. Using Eq. [...] $\theta$ has a value of 0.52557 degrees. The true angular size of the sun is approximately 0.5266 degrees as given on Wikipedia [... reference], so the percentage difference with my value and the value on Wikipedia is 0.196 \%, which is very good.
